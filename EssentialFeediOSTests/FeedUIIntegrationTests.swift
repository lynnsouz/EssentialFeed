...
final class FeedUIIntegrationTests: XCTestCase {
    ...
    func test_loadFeedCompletion_rendersSuccessfullyLoadedFeed() {
        ...
        var array = [image0]
        loader.completeFeedLoading(with: array, at: 0)
        assertThat(sut, isRendering: array)

        array = [image0, image1, image2, image3]
        sut.simulateUserInitiatedFeedReload()
        loader.completeFeedLoading(with: array, at: 1)
        assertThat(sut, isRendering: array)
    }
    ...
    func test_feedImageViewRetryButton_isVisibleOnInvalidImageData() {
        ...
        XCTAssertEqual(view?.isShowingRetryAction, false,
                       "Expected no retry action while loading image")
    ...
    func test_loadImageDataCompletion_dispatchesFromBackgroundToMainThread() {
        ...
        let
        exp = expectation(description: "Wait for background queue")
        DispatchQueue.global().async {
    ...
    private func assertThat(_ sut: FeedViewController,
        ...
        XCTAssertEqual(cell.isShowingLocation,
    ...
    private func localized(_ key: String,
        ...
        let value = bundle.localizedString(forKey: key,
                                           ...
                                           table: table)
        if value == key {
            XCTFail("Missing localized string for key: \(key) in table: \(String(describing: table))",
                    file: file,
                    line: line)
        }
        return value
